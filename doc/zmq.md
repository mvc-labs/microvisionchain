...
The MVC daemon can be configured to act as a trusted "border
router", implementing the mvc wire protocol and relay, making
...
The ZeroMQ feature in MVC requires ZeroMQ API version 4.3.2 or
...
during the *configure* step of building mvcd:
...
For instance:
    ...
    $ mvcd -zmqpubhashtx=tcp://127.0.0.1:29882 \
               -zmqpubrawtx=ipc:///tmp/mvcd.tx.raw

These options can also be provided in mvc.conf.
...
From the perspective of mvcd, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into MVC directly. Furthermore, no information is
...
using. MVC appends an up-counting sequence number to each
