...
std::string HelpMessage(HelpMessageMode mode, const Config& config) {
    ...
    strUsage += HelpMessageOpt(
        "-pid=<file>",
        strprintf(_("Specify pid file (default: %s)"), MVC_PID_FILENAME));
#endif
    ...
    strUsage += HelpMessageOpt(
        "-printtoconsole",
        _("Send trace/debug info to console instead of mvcd.log file"));
    strUsage += HelpMessageOpt("-shrinkdebugfile",
                               _("Shrink mvcd.log file on client startup "
...
std::string LicenseInfo() {
    const std::string URL_SOURCE_CODE =
        "<https://github.com/mvc-labs/microvisionchain>";
    const std::string URL_WEBSITE = "<https://www.microvisionchain.com>";
...
/** Sanity checks
 *  Ensure that MVC is running in a usable environment with all
...
static bool LockDataDirectory(bool probeOnly) {
    ...
    // Make sure only a single MVC process is using the data directory.
...
bool AppInitMain(ConfigInit &config, boost::thread_group &threadGroup,
    ...
    if (gArgs.GetBoolArg("-shrinkdebugfile", default_shrinkdebugfile)) {
        // Do this first since it both loads a bunch of mvcd.log into memory,
        // and because this needs to happen before any other mvcd.log printing.
    ...
    LogPrintf(
        "Using config file %s\n",
        GetConfigFile(gArgs.GetArg("-conf", MVC_CONF_FILENAME)).string());
