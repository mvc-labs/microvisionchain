...
static int AppInitRawTx(int argc, char *argv[]) {
    ...
    if (argc < 2 || gArgs.IsArgSet("-?") || gArgs.IsArgSet("-h") ||
        ...
        std::string strUsage =
            strprintf(_("%s mvc-tx utility version"), _(PACKAGE_NAME)) +
            " " + FormatFullVersion() + "\n\n" + _("Usage:") + "\n" +
            "  mvc-tx [options] <hex-tx> [commands]  " +
            _("Update hex-encoded mvc transaction") + "\n" +
            "  mvc-tx [options] -create [commands]   " +
            _("Create hex-encoded mvc transaction") + "\n" + "\n";
...
static void MutateTxAddInput(CMutableTransaction &tx,
    ...
    static const unsigned int maxVout = MAX_TX_SIZE_CONSENSUS_AFTER_GENESIS / minTxOutSz; // mvc-tx tool can build txs with more vouts than pre Genesis nodes  would accept
...
static int CommandLineRawTx(int argc, char *argv[],
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded mvc transaction
