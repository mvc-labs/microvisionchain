...
static UniValue getinfo(const Config &config, const JSONRPCRequest &request) {
    if (request.fHelp || request.params.size() != 0) {
        throw std::runtime_error(
            ...
            "  \"balance\": xxxxxxx,         (numeric) the total mvc "
...
#ifdef ENABLE_WALLET
        // Case 1: MVC address and we have full public key:
        if (pwallet) {
...
static UniValue createmultisig(const Config &config,
    ...
    if (request.fHelp || request.params.size() < 2 ||
        ...
        std::string msg =
            ...
            "are mvc addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"key\"    (string) mvc address or hex-encoded public "
            "key\n"
            "       ,...\n"
            "     ]\n"
...
static UniValue echo(const Config &config, const JSONRPCRequest &request) {
    if (request.fHelp) {
        throw std::runtime_error(
            ...
            "mvc-cli. There is no server-side difference.");
    }
