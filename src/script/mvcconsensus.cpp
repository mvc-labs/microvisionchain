...
inline int set_error(mvcconsensus_error *ret,
                     mvcconsensus_error serror) {
    if (ret) *ret = serror;
    return 0;
}
...
static bool verify_flags(unsigned int flags) {
    return (flags & ~(mvcconsensus_SCRIPT_FLAGS_VERIFY_ALL)) == 0;
}
...
static int verify_script(const CScriptConfig& config, const uint8_t* scriptPubKey,
                         ...
                         mvcconsensus_error* err) {
    if (!verify_flags(flags)) {
        return mvcconsensus_ERR_INVALID_FLAGS;
    }
    try {
        TxInputStream stream(SER_NETWORK, PROTOCOL_VERSION, txTo, txToLen);
        CTransaction tx(deserialize, stream);
        if (nIn >= tx.vin.size())
            return set_error(err, mvcconsensus_ERR_TX_INDEX);
        if (GetSerializeSize(tx, SER_NETWORK, PROTOCOL_VERSION) != txToLen)
            return set_error(err, mvcconsensus_ERR_TX_SIZE_MISMATCH);

        // Regardless of the verification result, the tx did not error.
        set_error(err, mvcconsensus_ERR_OK);
        ...
        // Error deserializing
        return set_error(err, mvcconsensus_ERR_TX_DESERIALIZE);
    }
}

int mvcconsensus_verify_script_with_amount(
...
int mvcconsensus_verify_script(const CScriptConfig& config,
                                   const uint8_t *scriptPubKey,
                                   unsigned int scriptPubKeyLen,
                                   const uint8_t *txTo, unsigned int txToLen,
                                   unsigned int nIn, unsigned int flags,
                                   mvcconsensus_error *err) {
    if (flags & mvcconsensus_SCRIPT_ENABLE_SIGHASH_FORKID ||
        flags & mvcconsensus_SCRIPT_FLAGS_VERIFY_WITNESS_DEPRECATED) {
        return set_error(err, mvcconsensus_ERR_AMOUNT_REQUIRED);
    }
...
unsigned int mvcconsensus_version() {
    // Just use the API version for now
    return MVCCONSENSUS_API_VER;
}
