...
uint64_t CScript::GetSigOpCount(bool fAccurate, bool isGenesisEnabled, bool& sigOpCountError) const
{
    ...
    uint64_t n = 0;
    mvc::instruction last_instruction{OP_INVALIDOPCODE};
...
bool IsP2SH(const mvc::span<const uint8_t> script) {
...
bool IsDSNotification(mvc::span<const uint8_t> script) {
...
bool IsDustReturnScript (mvc::span<const uint8_t> script)
{
...
CScript &CScript::push_int64(int64_t n) {
    if (n == -1 || (n >= 1 && n <= 16)) {
        ...
        v.reserve(sizeof(n));
        mvc::serialize(n, back_inserter(v));
        *this << v;
    }
    return *this;
}
...
mvc::instruction_iterator CScript::begin_instructions() const
{
    return mvc::instruction_iterator{mvc::span<const uint8_t>{data(), size()}};
}

mvc::instruction_iterator CScript::end_instructions() const
{
    return mvc::instruction_iterator{
        mvc::span<const uint8_t>{data() + size(), 0}};
}
