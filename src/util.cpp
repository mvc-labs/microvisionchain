...
const char *const MVC_CONF_FILENAME = "mvc.conf";
const char *const MVC_PID_FILENAME = "mvcd.pid";
...
// Windows < Vista: C:\Documents and Settings\Username\Application Data\MVC
// Windows >= Vista: C:\Users\Username\AppData\Roaming\MVC
// Mac: ~/Library/Application Support/MVC
// Unix: ~/.mvc
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "MVC";
#else
    fs::path pathRet;
...
#ifdef __APPLE__
    // Mac
    return pathRet / "Library/Application Support/MVC";
#else
    // Unix
    return pathRet / ".mvc";
#endif
...
#ifndef WIN32
fs::path GetPidFile() {
    fs::path pathPidFile(gArgs.GetArg("-pid", MVC_PID_FILENAME));
