...
static UniValue getaccountaddress(const Config &config,
    ...
    if (request.fHelp || request.params.size() != 1) {
        throw std::runtime_error(
            ...
            "\nDEPRECATED. Returns the current MVC address for receiving "
...
static UniValue setaccount(const Config &config,
    ...
    if (request.fHelp || request.params.size() < 1 ||
        ...
        throw std::runtime_error(
            ...
            "address.\n"
            "\nArguments:\n"
            "1. \"address\"         (string, required) The mvc address to "
...
static UniValue getaccount(const Config &config,
    ...
    if (request.fHelp || request.params.size() != 1) {
        throw std::runtime_error(
            ...
            "address.\n"
            "\nArguments:\n"
            "1. \"address\"         (string, required) The mvc address for "
            "account lookup.\n"
            "\nResult:\n"
...
static UniValue getaddressesbyaccount(const Config &config,
    ...
    if (request.fHelp || request.params.size() != 1) {
        throw std::runtime_error(
            ...
            "  \"address\"         (string) a mvc address associated with "
            "the given account\n"
            "  ,...\n"
            "]\n"
            "\nExamples:\n" +
...
static UniValue sendtoaddress(const Config &config,
    ...
    if (request.fHelp || request.params.size() < 2 ||
        ...
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. \"address\"            (string, required) The mvc address "
            "to send to.\n"
...
static UniValue sendfrom(const Config &config, const JSONRPCRequest &request) {
    ...
    if (request.fHelp || request.params.size() < 3 ||
        ...
        throw std::runtime_error(
            ...
            "reflect\n"
            "                       the spend.\n"
            "2. \"toaddress\"         (string, required) The mvc address "
            "to send funds to.\n"
...
static UniValue sendmany(const Config &config, const JSONRPCRequest &request) {
    ...
    if (request.fHelp || request.params.size() < 2 ||
        ...
        throw std::runtime_error(
            ...
            "    {\n"
            "      \"address\":amount   (numeric or string) The mvc "
    ...
    for (const std::string &name_ : keys) {
        ...
        if (!IsValidDestination(dest)) {
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY,
                               std::string("Invalid MVC address: ") +
                                   name_);
        }
...
static UniValue addmultisigaddress(const Config &config,
    ...
    if (request.fHelp || request.params.size() < 2 ||
        ...
        std::string msg =
            ...
            "2. \"keys\"         (string, required) A json array of mvc "
            "addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"address\"  (string) mvc address or hex-encoded "
            "public key\n"
            "       ...,\n"
            "     ]\n"
...
static UniValue listtransactions(const Config &config,
    ...
    if (request.fHelp || request.params.size() > 4) {
        throw std::runtime_error(
            ...
            "    \"address\":\"address\",    (string) The mvc address of "
...
static UniValue gettransaction(const Config &config,
    ...
    if (request.fHelp || request.params.size() < 1 ||
        ...
        throw std::runtime_error(
            ...
            "default account.\n"
            "      \"address\" : \"address\",          (string) The mvc "
...
static UniValue encryptwallet(const Config &config,
    ...
    StartShutdown();
    return "wallet encrypted; MVC server stopping, restart to run with "
...
static UniValue listunspent(const Config &config,
    ...
    if (request.fHelp || request.params.size() > 4) {
        throw std::runtime_error(
            ...
            "3. \"addresses\"    (string) A json array of mvc addresses to "
            "filter\n"
            "    [\n"
...
static UniValue fundrawtransaction(const Config &config,
    ...
    if (request.fHelp || request.params.size() < 1 ||
        ...
        throw std::runtime_error(
            ...
            "address) The mvc address to receive the change\n"
            ...
            if (options.exists("changeAddress")) {
                ...
                if (!IsValidDestination(dest)) {
                    throw JSONRPCError(
                        ...
                        "changeAddress must be a valid mvc address");
                }
